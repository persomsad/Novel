...
- ✅ **知识图谱检索**：基于 NervusDB 的图数据库，智能关联角色关系、时间线、伏笔链
- ✅ **实时上下文理解** ⭐ 新功能：自动选择相关文档，像 Claude Code 一样智能
- ✅ **会话持久化**：保存创作历史，支持多轮对话
...
# 禁用文件监控（减少资源占用）
novel-agent chat --disable-watcher
...
Agent: [自动加载]
  ✓ 自动注入上下文: 3 个文档
    ...
    - chapters/ch001.md (前置章节)

Agent 🟢 置信度: 85/100
[推理分析...]
✅ 检查完成！发现 1 个问题：
   - 第3章第15行：主角性格描写与设定不一致
   - 设定：善良但缺乏自信
   - 实际：表现得非常勇敢和果断
   - 建议：调整描写或修改设定
```
...
**置信度评分说明**：
- 🟢 80-100: 高质量回答（结构化、有具体建议、有文件引用）
- 🟡 60-79: 中等质量（有一定结构，但可能缺少细节）
- 🔴 0-59: 低质量（过短、空洞或包含大量错误标记）
