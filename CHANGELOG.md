...
#### 简化权限系统 (#73)
- **2级权限系统**: default（所有工具）vs `--read-only`（仅分析工具）
- **向后兼容**: 旧参数保留但标记为 deprecated
- **清晰分类**: 明确区分只读工具和修改工具
- **移除**: `--allowed-tools`, `--disallowed-tools`, `--tools` 参数（过度设计）

#### 提示词文件支持 (#74)
- **文件加载**: `-f, --file` 参数加载 Markdown 格式 prompt
- **变量替换**: `--var KEY=VALUE` 支持 `${VAR_NAME}` 占位符
- **Frontmatter**: 支持 YAML frontmatter 元数据
- **示例模板**: 5 个内置模板
  - `check-consistency.md`: 一致性检查
  - `analyze-plot.md`: 情节分析
  - `suggest-next-chapter.md`: 下章建议
  - `character-relationship.md`: 角色关系
  - `foreshadow-check.md`: 伏笔检查
- **集成**: 支持 print 模式和交互模式

#### 终端输入位置优化 (#75)
- **自动计算**: 输入框位于屏幕 65% 位置
- **手动控制**: `--input-offset` 参数自定义偏移
- **范围限制**: 3-10 行范围，避免过度偏移
- **更好体验**: 视线不用来回移动，更符合阅读习惯

#### 会话压缩功能 (#76)
- **LLM 摘要**: 使用 LLM 生成会话摘要
- **Token 优化**: 中文优化（1.5 字符/token）
- **压缩命令**: `/compress [新提示]` 压缩当前会话
- **统计信息**: 显示原始/压缩 tokens 和节省率
- **自动切换**: 压缩后自动切换到新会话
- **支持立即执行**: `/compress 继续讨论情节` 压缩并执行新提示
- **异步支持**: 安装 trio 依赖，支持 asyncio + trio 双后端测试

#### 会话管理功能 (#77)
- **查看历史**: `/sessions` 命令列出最近 10 个会话
- **元数据显示**: 时间、标题、轮次、token 数
- **会话切换**: 选择序号切换到历史会话
- **导出功能**: `/export [格式]` 导出当前会话
- **多格式支持**:
  - Markdown（默认）: 适合阅读
  - JSON: 适合程序处理
  - Text: 纯文本格式
- **自动路径**: 默认导出到 `exports/` 目录
...
- 新增 10 个会话导出测试
- 新增 19 个会话压缩测试（asyncio + trio）
- 新增 11 个 prompt 加载测试
- 新增 8 个终端偏移测试
- 新增 3 个权限测试
- **总测试数**: 270 个（269 passed, 1 skipped）
- **覆盖率**: 66.80%

### 🔧 依赖

- 新增: `python-frontmatter` (prompt 文件解析)
- 新增: `trio` (dev, 异步测试支持)
